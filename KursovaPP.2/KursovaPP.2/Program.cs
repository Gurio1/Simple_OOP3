...
namespace KursovaPP._2
{
   ...
   static class Program
        {
        static List<Galaxy> Galaxyes = new List<Galaxy>();

        static void AddStar(string nameOfGalaxy,string nameOfStar,double mass,double size,double temp, double luminosity)
        {
            char StarClass = SetClass();
            if (StarClass == ('0'))
            {
            ...
            char SetClass()
            {
                var RezultOfFilter = filters.Select(filter => filter(mass, size/2, temp, luminosity)).First(x=>x.Item1);
                return RezultOfFilter.Item2;
            }
        }
        ...
        static void ListOF(string objects)
        {
            switch (objects)
            {
                case "galaxies":
                ...
                case "moons":
                    ...
                    void PrintList(string[] Names, string obj)
                    {
                        StringBuilder sb = new StringBuilder();
